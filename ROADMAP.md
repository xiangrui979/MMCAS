...
> 当前版本：**v1.0（全链路内测版）**　下一目标：**v1.2 · 实战修订**

## v1.2 · 实战修订（进行中）

- [ ] **调度核心**：面板合并（自动接取轮询器 + 看板 30s git 同步）、修复重复/过期派发、去掉 WIP 硬限制（依赖门控为准）、补齐轮询器回归测试
- [ ] **会话与上下文**：启用 dsh 内置压缩（compaction）三件套；新任务自动开新会话接取、打回回原会话续做；上下文水位警示
- [ ] **跨端通道与治理**：消息层（裁决请求 / 错误上报 / 停止指令）、回程卡（成员 → Master 的问题卡）、打回机制强化（原因必填 + 通知 + 追溯）
- [ ] **独立审计实例**：一键对数学模型 / 代码 / 论文成品做第三方独立审计（较强专家模型，经本地桥）
- [ ] **同步基建**：sync-daemon v0.2.1 收口与下发、迁移自检（repo 配置刷新）、守护看门狗、凭据额度可见性
- [ ] **打包发布**：出厂默认项统一（模型目录 / 视觉能力）、三端重打包与升级流程验证
